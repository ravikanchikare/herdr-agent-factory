...
// Production and CI resolve the pinned SDK installed by pnpm. A checkout can
// still be selected explicitly with -Dnative-sdk-path or NATIVE_SDK_PATH.
const default_native_sdk_path = "../../node_modules/@native-sdk/cli";
const app_exe_name = "agent-factory";
...
pub fn build(b: *std.Build) void {
    ...
    if (web_engine != .system) {
        @panic("Agent Factory uses the system WebView; Chromium is not supported");
    }
    ...
    b.installFile("herdr-client.toml", "bin/herdr-client.toml");
    ...
    const package_arch = packageArchitecture(target);
    const package_output = b.fmt(
        "zig-out/package/{s}-0.1.0-{s}-{s}-{s}{s}",
        .{
            app_exe_name,
            @tagName(package_target),
            package_arch.bundle_name,
    ...
    if (package_target == .macos) {
        const generate_update_config = b.addSystemCommand(&.{
            "cargo",
            "run",
            "--locked",
            "--manifest-path",
            ...
            "--output",
            b.fmt(
                "{s}/Contents/Resources/update-config-v1.json",
                .{package_output},
            ),
        });
        generate_update_config.step.dependOn(&package.step);

        const runtime_build = b.addSystemCommand(&.{
            "cargo",
            "build",
            "--manifest-path",
            "../../Cargo.toml",
            ...
            "--release",
            "--target",
            package_arch.rust_target,
        });
        runtime_build.setEnvironmentVariable("MACOSX_DEPLOYMENT_TARGET", "13.0");
        const install_runtime = b.addSystemCommand(&.{
            "/usr/bin/install",
            "-m",
            "755",
        });
        install_runtime.addFileArg(b.path(
            b.fmt(
                "../../target/{s}/release/agent-factory-runtime",
                .{package_arch.rust_target},
            ),
        ));
        install_runtime.addArg(b.fmt(
            "{s}/Contents/Resources/agent-factory-runtime",
            .{package_output},
        ));
        install_runtime.step.dependOn(&package.step);
        install_runtime.step.dependOn(&runtime_build.step);

        const install_updater = b.addSystemCommand(&.{
            "/usr/bin/install",
            "-m",
            "755",
        });
        install_updater.addFileArg(b.path(
            b.fmt(
                "../../target/{s}/release/updater-helper",
                .{package_arch.rust_target},
            ),
        ));
        install_updater.addArg(b.fmt(
            "{s}/Contents/Resources/updater-helper",
            .{package_output},
        ));
        install_updater.step.dependOn(&package.step);
        install_updater.step.dependOn(&runtime_build.step);

        const install_herdr_config = b.addSystemCommand(&.{
            "/usr/bin/install",
            "-m",
            "644",
            "herdr-client.toml",
            b.fmt(
                "{s}/Contents/Resources/herdr-client.toml",
                .{package_output},
            ),
        });
        install_herdr_config.step.dependOn(&package.step);

        // A macOS app cannot recover from a mixed-architecture host and
        // sidecar. Verify the exact single-architecture bundle before any
        // signature makes it look release-ready.
        const verify_architecture = b.addSystemCommand(&.{
            "/bin/bash",
            "scripts/verify-macos-bundle.sh",
            package_output,
            package_arch.lipo_name,
        });
        verify_architecture.step.dependOn(&install_runtime.step);
        verify_architecture.step.dependOn(&install_updater.step);
        verify_architecture.step.dependOn(&install_herdr_config.step);
        verify_architecture.step.dependOn(&generate_update_config.step);

        // Native-SDK 0.5 writes an 11.0 plist floor even when the executable
        // target is macOS 13. Keep the package metadata aligned with the two
        // binaries before applying the final outer signature.
        const set_minimum_macos = b.addSystemCommand(&.{
            "/usr/bin/plutil",
            "-replace",
        ...
        set_minimum_macos.addArg(b.fmt(
            "{s}/Contents/Info.plist",
            .{package_output},
        ));
        set_minimum_macos.step.dependOn(&verify_architecture.step);

        const sign_runtime = b.addSystemCommand(&.{
            "/usr/bin/codesign",
            "--force",
            "--options",
            "runtime",
        ...
        sign_runtime.addArg(b.fmt(
            "{s}/Contents/Resources/agent-factory-runtime",
            .{package_output},
        ));
        sign_runtime.step.dependOn(&verify_architecture.step);

        const sign_updater = b.addSystemCommand(&.{
            "/usr/bin/codesign",
            "--force",
            "--options",
            "runtime",
        ...
        sign_updater.addArg(b.fmt(
            "{s}/Contents/Resources/updater-helper",
            .{package_output},
        ));
        sign_updater.step.dependOn(&verify_architecture.step);

        // This is a deterministic local ad-hoc signature. The release
        // pipeline replaces it with Developer ID signatures, in the same
        // inner-runtime-then-outer-bundle order, before notarization.
        ...
        sign_bundle.addArg(package_output);
        sign_bundle.step.dependOn(&set_minimum_macos.step);
        sign_bundle.step.dependOn(&sign_runtime.step);
        sign_bundle.step.dependOn(&sign_updater.step);
        ...
        verify_signature.addArg(package_output);
        verify_signature.step.dependOn(&sign_bundle.step);
        package_step.dependOn(&verify_signature.step);
    } else {
    ...
    const terminal_tests = b.addTest(.{
        ...
        .filters = &.{
            "terminal_session_tests",
            "right click bypasses the native menu while a terminal captures mouse input",
        },
        .root_module = b.createModule(.{
            .root_source_file = nativeSdkPath(
                b,
                native_sdk_path,
                "src/agent_factory_terminal_tests_root.zig",
            ),
            .target = target,
            .optimize = optimize,
        }),
    });
    var terminal_test_imports = native_sdk_mod.import_table.iterator();
    while (terminal_test_imports.next()) |entry| {
        terminal_tests.root_module.addImport(entry.key_ptr.*, entry.value_ptr.*);
    }
...
fn nativeSdkModule(b: *std.Build, target: std.Build.ResolvedTarget, optimize: std.builtin.OptimizeMode, native_sdk_path: []const u8) *std.Build.Module {
    ...
    native_sdk_mod.addImport("terminal_vt", terminal_vt: {
        if (b.lazyDependency("ghostty", .{
            ...
            const wrapper = externalModule(
                b,
                target,
                optimize,
                native_sdk_path,
                "src/runtime/terminal_vt_ghostty.zig",
            );
            wrapper.addImport("ghostty-vt", ghostty.module("ghostty-vt"));
            break :terminal_vt wrapper;
        }
        break :terminal_vt externalModule(
            b,
            target,
            optimize,
            native_sdk_path,
            "src/runtime/terminal_vt_stub.zig",
        );
    });
...
const PackageArchitecture = struct {
    bundle_name: []const u8,
    lipo_name: []const u8,
    rust_target: []const u8,
};

fn packageArchitecture(target: std.Build.ResolvedTarget) PackageArchitecture {
    return switch (target.result.cpu.arch) {
        ...
        else => @panic("macOS packages support only arm64 and x86_64"),
    };
}
